...
QuantMdHandler::QuantMdHandler(CThostFtdcMdApi *mdapi)
{
    requestID = 0;
    this->mdapi = mdapi;
}

void QuantMdHandler::setApi(QuantMdUser* api){
    this->api = api;
}

// 允许登录事件
void QuantMdHandler::OnFrontConnected()
{
...
///        0x2002 发送心跳失败
///        0x2003 收到错误报文
void QuantMdHandler::OnFrontDisconnected(int nReason)
{
...
void QuantMdHandler::OnHeartBeatWarning(int nTimeLapse)
{
    printf("OnHeartBeatWarning %d\n", nTimeLapse);
}

// 登录结果响应
void QuantMdHandler::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin,
...
// 登出结果响应
void QuantMdHandler::OnRspUserLogout(CThostFtdcUserLogoutField *pUserLogout,
                                 CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
...
// 错误信息响应方法
void QuantMdHandler::OnRspError(CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    char *msg = ExtGbkToUtf8(pRspInfo->ErrorMsg);
...
// 行情详情通知
void QuantMdHandler::OnRtnDepthMarketData(CThostFtdcDepthMarketDataField *pDepthMarketData)
{
	if (!this->api)
		return;
